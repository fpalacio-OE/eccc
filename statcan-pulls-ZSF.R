...
# returns: dataframe
#
download_data <- function(vectorIds, start_time, end_time) {
  vectorIds <- vectorIds[!is.na(vectorIds)]
  # select resource endpoint, in this case /download

  endpoint <- "/getBulkVectorDataByRange"
  
  vectors_string = paste0("\"vectorIds\":[", paste(purrr::map(as.character(vectorIds),
                                         function(x) paste0("\"", x, "\"")), collapse = ", "),
                          "]")
  
  time_string = paste0("\"startDataPointReleaseDate\": \"",
                       strftime(start_time, time_format), "\",\"endDataPointReleaseDate\": \"",
                       strftime(end_time, time_format), "\"")
  
  new_postBody <- paste0("{", vectors_string, ",", time_string, "}")
...
### Parameters------------------------------------------------------####
srcDir <- "C:/Users/nycvideo/Desktop/EnvCan/resources/"
dstDir <- "C:/Users/nycvideo/Desktop/EnvCan/outputs/"
exportname <- "envcandb"
...
startDate <- strftime(start_time, time_format)
end_time <- Sys.Date()
endDate <- strftime(end_time, time_format)
...
### Commands----------------------------------------------------####
#
data2<-mapply(download_data, vectors, start_time,end_time)
