...
download_metadata_pID <- function(product_ID){
  ...
  for(i in product_ID){
    ...
    postBody <- paste("[",toJSON(list(
      productId = i)
      , auto_unbox = TRUE, pretty = T),"]",sep="")
...
final_cleaning<-function(final){
  ...
  #extract prices
  price_tag<-as.data.table(which(Vectorize(function(x) x %in% c("Prices","Value"))(final), arr.ind=TRUE))
  ...
  final[info %like% "price indexes" | memberNameEn3 %in% "Non-residential structures, machinery and equipment"|cansimId %in% c("3850042","3850033","3850038"), new_var:=variable]
  ...
  sum<-dcast(dt2, cansimId+geography+var+code+tag+year+productId+scalarFactorCode+prices+variable+industry+cubeTitleEn+term+sa~ ., value.var = "value", fun.aggregate = sum)
  ...
  year     <- rep(kronecker(1955:2017, rep(1,nind)),ngeo)
...
aggregation_codes[,Vector:=as.numeric(gsub("V","",Vector))]
