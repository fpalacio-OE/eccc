...
#key functions
source("C:/Users/Fabio Palacio.OEF/OneDrive - Oxford Economics/envcan/code/finalversion/imposegrowthrates.r")
source("C:/Users/Fabio Palacio.OEF/OneDrive - Oxford Economics/envcan/code/finalversion/verticaltieupfunction.r")
source("C:/Users/Fabio Palacio.OEF/OneDrive - Oxford Economics/envcan/code/finalversion/simpletieupfunction.r")
...
for(i in 1:length(finalvariables)){
  ...
  broken<-lapply(broken,function(broken){
    ...
    for(j in 4:ncol(broken)){
      #print(j)
      indseries<-broken[[j]]
      if(all(is.na(indseries))|all(!is.na(indseries))){next}
...
finalprep<-function(x){
  ...
  x[, (newnames):= lapply(foroperation, function(x) {c(NA,exp(diff(log(get(x)))))}), by=list(geography, code)]
}
...
#mark the cells that are in the 379-0030 (1 means it's  in 379-0030 and therefore fixed)
`gva - chained prices (x 1,000,000)-subgroup`[geography!="Canada", tag:=ifelse(is.na(`gross domestic product (gdp) at basic prices-3790030-chained (2007) dollars`), 0,1)]
`gva - chained prices (x 1,000,000)-subgroup`[geography=="Canada", tag:=ifelse(is.na(`gross domestic product (gdp) at basic prices-3790031-chained (2007) dollars`), 0,1)]
...
extendgva<- function(x,i){
  ...
  DT[level ==i,(var):= lapply(newcols,function(y){get(y)})]#[,c(umbrellagrowth,umbrellavalues):=NULL]
}
...
for(i in 1:5){
  ...
  forgvaextension[,(umbrellavalues):= lapply(var, function(var) {get(var)[match(umbrella,code,nomatch=NA)]}),
  ...
  suppressWarnings(forgvaextension[,(umbrellagrowth):=lapply(umbrellavalues, function(umbrellavalues) {c(NA,exp(diff(log(get(umbrellavalues)))))}),by=code])
...
nationalgva[,     `:=`
                  ...
                  `:=`
                    (natlsharenom   =ifelse(is.na(`gva - current prices (x 1,000,000)`/umbrellavalnom), NA, `gva - current prices (x 1,000,000)`/umbrellavalnom),
                     ...
                     marker       :=all(is.na(natlsharereal)),by=code][,
                     natlsharereal:=ifelse(marker==TRUE, natlsharenom,natlsharereal), by=code][,
...
#for inspection
setcolorder(remaininggvagaps, c("year", "code","level", "geography", "gva - current prices (x 1,000,000)", 
...
remaininggvagaps[,c(  "umbrella", "natlvalnom", "natlumbvalnom", "natlsharenom",
                      "umbrellavalnom","natlvalreal", "natlumbvalreal", "natlsharereal","umbrellavalreal","level"):=NULL]
...
forgrowback<-merge(forgrowback,`gva - chained prices (x 1,000,000)-subgroup`[,c("geography", "code", "year", "gross domestic product (gdp) at basic prices-3790031-chained (2007) dollars")], by=c("code", "year", "geography"))
forgrowback[geography=="Canada",tagreal :=ifelse(all(is.na(`gross domestic product (gdp) at basic prices-3790031-chained (2007) dollars`)),0,1),by=list(code,geography)][,`gross domestic product (gdp) at basic prices-3790031-chained (2007) dollars`:=NULL]
...
test2[year==2007,try:=`gva - current prices (x 1,000,000)`]
...
primary<-envcandbshort[,c("code", "year", "geography", "gross domestic product (gdp) at basic prices-3790030-current prices", "gross domestic product (gdp) at basic prices-3790029-current prices", "gross domestic product (gdp) at basic prices-3790031-chained (2007) dollars","gross domestic product (gdp) at basic prices-3790030-chained (2012) dollars"),with=F]
primary[geography=="Canada",`:=`
        (primaryreal= `gross domestic product (gdp) at basic prices-3790031-chained (2007) dollars`,
...
variablescurr<-c("employment", "gva - current prices (x 1,000,000)", "gross output (x 1,000,000)", 
...
for(i in 1:5){
  ...
  print(var)
    ...
    GVAallocation[, `:=`
                    ...
                    umbrellaval=get(var)[match(umbrella,code,nomatch=NA)]),
                    by=list(year,geography)][, try:= ifelse(is.na(get(var)),umbrellaval*share, get(var)) ]
...
#now real
variableschained<-c("capital stock - chained prices (x 1,000,000)",
...
finaltiesformerge<-mapply(verthortieup,fixedseriesfinaltieup, tagsforfinaltieup, gvatag=0, SIMPLIFY = F)
#regroup
...
fulldbmelted[,`:=`
           (mnemonic  = paste(varsymbol, sectormnems,pricesymbol,sep=""))
            ][, c("pricesymbol", "varsymbol", "sectormnems"):=NULL]
