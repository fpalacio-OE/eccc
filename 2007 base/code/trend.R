...
#reshape & fix name
dt<-melt(dt, id.vars = c("sectorcode", "variable", "ind"),variable.name = "year")
...
for(i in seq_along(DT2)){
  if(all(is.na(DT2[[i]]))){missing<-c(missing, names(DT2)[i])}
}
...
#kill early NAs
trend[,test:=apply(.SD, 1, function(x) min(which(!is.na(x)))),.SD=(yearcols)]
#replace
