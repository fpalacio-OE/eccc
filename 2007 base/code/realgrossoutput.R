...
adj <- as.numeric(unique(foravg[year == 2007, 100 - final]))
...
# apply the growth rate to the nominal series, 2007 forward and 2007 backwards

output[year == 2007, try := `gross output (x 1,000,000)`]
output <- output[order(geography, code, year)]
for (y in (2008):max(output$year)) {
  # print(y)
  suppressWarnings(
...
GVAallocation[year == 2007, `gross output - constant prices (x 1,000,000)` := `gross output (x 1,000,000)`][, share := ifelse(`gross output - constant prices (x 1,000,000)` == 0 | `gva - chained prices (x 1,000,000)` == 0, 0,
  `gross output - constant prices (x 1,000,000)` / `gva - chained prices (x 1,000,000)`
)]
# set them constant over the time period
formatch <- GVAallocation[year == 2007, .(code, geography, share)]
...
forgrowth[, growth := c(NA, exp(diff(log(`gva - chained prices (x 1,000,000)`)))), by = list(geography, code)]
...
# apply
rgrossoutputtable[year != 2007 & geography != "Canada", `gross output - constant prices (x 1,000,000)` := try2]
